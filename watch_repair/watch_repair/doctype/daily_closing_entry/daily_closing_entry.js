        ...
        frappe.call({
            ...
            callback: function(response) {
                if (response.message && response.message.length > 0) {
                    ...
                    frm.clear_table('daily_closing_mop'); 
                                            ...
                                            callback: function(item_response) {
                                                if (item_response.message) {
                                                    ...
                                                    if (items && items.length > 0) {
                                                        ...
                                                        total_item_group_amount += items[0].amount || 0;
    
                                                        frm.set_value('total_amount', total_item_group_amount);
                                                    }
...
posting_date:function(frm){
    ...
    if (frm.doc.posting_date && frappe.datetime.get_diff(frm.doc.posting_date, frappe.datetime.now_date()) > 0) {
        frappe.msgprint(__('Posting date cannot be a future date'));
